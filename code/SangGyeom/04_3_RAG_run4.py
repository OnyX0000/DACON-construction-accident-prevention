...
def get_prompt_template():
    ...
    ### 지침: 당신은 건설 안전 전문가입니다.
    - 상황과 질문을 보고 참고 문맥을 활용하여 재발방지대책 및 향후조치계획을 답변하세요.
    - 답변은 반드시 핵심 용어와 주요 문구만을 사용하여 작성해야 합니다.
    - 서론, 배경 설명, 부연 설명은 절대 포함하지 마세요.
    - 답변은 간결하고 명확하게 100토큰 내외로 간결하게 작성하세요.
    - 콤마(,) 등의 특수기호는 최대한 적게 사용하여 한 문장으로 작성하세요.
    
    - 올바른 답변의 어조의 예시는 다음과 같습니다:
    "추위가 강한 시간대에 공사를 가급적 자제하도록 유도하고, 피치 못할 사정으로 드럼통 사용 시 인화성이 강한 물질은 소각 금지하도록 교육하는 방안."
...
def get_relevant_context(vectordb, query, k=5):
    ...
    print(f"가장 유사도가 높은 문서 제목: '{top_title}', 유사도: {top_score:.4f}")
...
def run_rag_inference():
    ...
    generated_file_path = "./code/SangGyeom/data/generated_sentences0323_3.csv"
    ...
    for idx, row in tqdm(test_df.iterrows(), total=len(test_df), desc="RAG 추론"):
        ...
        status = f"공사종류 대분류 '{row['공사종류(대분류)']}', 중분류 '{row['공사종류(중분류)']}' 공사 중 "
        f"공종 대분류 '{row['공종(대분류)']}', 중분류 '{row['공종(중분류)']}' 작업에서 "
        f"사고객체 '{row['사고객체(대분류)']}'(중분류: '{row['사고객체(중분류)']}')와 관련된 사고가 발생했습니다. "
        f"작업 프로세스는 '{row['작업프로세스']}'입니다"

        # 메타데이터 필터링 로직이 적용된 문맥 추출
        context = get_relevant_context(vectordb, query)
        prompt = template.format(context=context, question=query, status=status)
...
def generate_embeddings_and_save():
    ...
    generated_file_path = "./code/SangGyeom/data/generated_sentences0323_3.csv"
    # 최종 제출 파일 경로
    submission_path = "./code/SangGyeom/data/submission0323_3.csv"
