...
initialize_rag_model = rag_module.initialize_rag_model
create_rag_chain = rag_module.create_rag_chain
...
def run_rag_test(train_csv, vector_db_dir, model_name, test_size=10):
    ...
    # 3. LLM 모델 초기화
    llm = initialize_rag_model(model_name)
    if llm is None:
        print("LLM 모델 초기화 실패")
        return
    ...
    csv_file_path = f"./code/SangGyeom/results/rag_test_results_{timestamp}.csv"
